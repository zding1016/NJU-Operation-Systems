...
void bootMain(void) {
	...
	kMainEntry=(void(*)(void))(((struct ELFHeader*)elf)->entry);//指向elf的entry
	phoff=((struct ELFHeader *)elf)->phoff;//指向程序头表
	offset=((struct ProgramHeader*)(elf+phoff))->off;//指向程序头表的offset
	for (i = 0; i < 200 * 512; i++) {
